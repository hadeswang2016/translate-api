...
/*
db_conf = ["dbhostsip","dbusername","dbpassword","dbname"]
*/
...
func get_trans_content(translator int,word_list []string{},db_conf map[string] DbConf)(res int) {
	
    /*for _,v:= range(word_list){
    	timestamp := time.Now().Unix()
    	db, err := sql.Open("mysql", "root:@tcp(127.0.0.1:3306)/hadtest?charset=utf8") 
    	checkErr(err)  
    	//插入数据  
    	var table string
    	if translator == 1{
    		table = "whattoexpect_youdao"
    	}else if translator == 2{
    		table = "whattoexpect_google"
    	}
	    stmt, err := db.Prepare("INSERT INTO "+table+" SET title=?,intro=?,content=?,image_cover_url=?,cur_link=?,ref_link=?,time=?")  
	    checkErr(err)  
	    res, err := stmt.Exec(nv[0], nv[1], nv[2],nv[3],nv[4],nv[5],timestamp)  
	    checkErr(err)  
